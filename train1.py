...
class AVNet(pl.LightningModule):
    ...
    def __init__(self, modal, W2Vfile, MoCofile, reqInpLen, modelargs):
        ...
        if not self.modal == "VO":
            # front-end
            wav2vecModel, cfg, task = fairseq.checkpoint_utils.load_model_ensemble_and_task([W2Vfile], arg_overrides={    # 这一句报错
                "apply_mask": True,
                "mask_prob": 0.5,
        ...
        if not self.modal == "AO":
            ...
            # params = list(visualModel.named_parameters())
            # print(params[0])  反正print的结果 加载完确实不一样了

            visualModel.load_state_dict(torch.load(MoCofile, map_location="cpu"), strict=False)   #加载了训练好的moco前端
            # params = list(visualModel.named_parameters())
            # print(params[0])
            #
            # exit(0)
    ...
    def subNetForward(self, inputBatch, maskw2v):
        ...
        if not self.modal == "AO":
            videoBatch = videoBatch.transpose(1, 2)  #（8，1，149，112，112）
            # if self.modal == "AV":
            #     with torch.no_grad():  #没有回传
            #         videoBatch = self.visualModel(videoBatch, vidLen.long())  #(927,2048)
            # else:
            videoBatch = self.visualModel(videoBatch, vidLen.long())    #（739,2048)
                                # vidLen tensor( [149, 70, 130,  51,  36, 143, 138,  22]  输入的图片序列长度, device='cuda:0')
            videoBatch = list(torch.split(videoBatch, vidLen.tolist(), dim=0))  #list:8   分别是 (149,2048) (70,2048)(130,2048) ...

        audioBatch, videoBatch, inputLenBatch, mask = self.makePadding(audioBatch, audLen, videoBatch, vidLen)
                                                 # none, (8,149,2048),   tensor([149, 70, 130,  51,  36, 143, 138,  22]  #mask:(8,149)
        if isinstance(self.maskedLayerNorm, MaskedLayerNorm):
            self.maskedLayerNorm.SetMaskandLength(mask, inputLenBatch)

        if not self.modal == "VO":
            # if self.modal == "AV":
            #     with torch.no_grad():   #(8,754,1024)   #不回传
        ...
        if not self.modal == "AO":
            ...
            # else:
            videoBatch = videoBatch.transpose(1, 2)  # (8,2048,149)
            videoBatch = self.videoConv(videoBatch)  #videoConv 就是 conv1dLayers （8，512，149）
            videoBatch = videoBatch.transpose(1, 2).transpose(0, 1)  #（149，8，512）
            videoBatch = self.EncoderPositionalEncoding(videoBatch)   #（149，8，512）
            videoBatch = self.videoEncoder(videoBatch, src_key_padding_mask=mask)  # videoEncoder 就是 nn.TransformerEncoder  （149，8，512）
    ...
    def forward(self, inputBatch, targetinBatch, targetLenBatch, maskw2v):  # tuple:4 ; (8,92) ; (8,) ; false
        ...
        jointCTCOutputBatch = jointBatch.transpose(0, 1).transpose(1, 2) #(8,512,512)  #AO: (8,512,152)     VO：（8，512，149）
        jointCTCOutputBatch = self.jointOutputConv(jointCTCOutputBatch)  #(8,40,152)  #AO: (8,40,152)       VO:
        jointCTCOutputBatch = jointCTCOutputBatch.transpose(1, 2).transpose(0, 1)  #(152,8,40)  #AO: (152,8,40)
        jointCTCOutputBatch = F.log_softmax(jointCTCOutputBatch, dim=2) #(152,8,40) #AO: (152,8,40)  ???152    VO:（149,8,40)  每一个输入对应一个输出字符 ！
        ...
        jointAttentionOutputBatch = jointAttentionOutputBatch.transpose(1, 2)  #(8,92,40)    VO:（8,98,40)  trgt 句子的字符数是98
    ...
    def training_step(self, batch, batch_idx):
        ...
        inputBatch, targetinBatch, targetoutBatch, targetLenBatch = batch  # inputBatch {list:4} (none,none,(8,149,1,112,112), [149, 70, 130,  51,  36, 143, 138,  22] )
       # t1 = time.time()
        Alpha = self.trainParams["Alpha"]   #(8,98) 补0补到98  #(8,98)  #tensor(8,)  [98, 45, 92, 25, 31, 59, 87, 15]
        #print(inputBatch[2].shape)
      ...
      #  t2 = time.time()
        ...
        # if t2-t1>=3:
        #     print("trian cost:",t2-t1)
        #     print(print(inputBatch[2].shape))

        # print("train cost",t2-t1)
        return loss
...
def main():
    #os.environ['CUDA_VISIBLE_DEVICES'] = '0,1'  #os.environ['CUDA_VISIBLE_DEVICES'] = '0'
    ...
    trainer = pl.Trainer(
        gpus=args["GPU_IDS"],    #max_epochs=50来设置 默认是1000
        max_epochs=2000,
        benchmark=False,
        deterministic=False,
        logger=writer,
    ...
    trainer.fit(model, LRS2Dataloader,ckpt_path="/data2/alumni/gryang/Leveraging-Self-Supervised-Learning-for-AVSR-main/check925/train-step_0107-wer_0.058.ckpt")
    #trainer.fit(model, LRS2Dataloader,ckpt_path="/data2/alumni/gryang/Leveraging-Self-Supervised-Learning-for-AVSR-main/train-step_0006-wer_0.065.ckpt")
    #trainer.fit(model, LRS2Dataloader,ckpt_path="/data2/alumni/gryang/Leveraging-Self-Supervised-Learning-for-AVSR-main/train-step_0011-wer_0.069.ckpt")
    #trainer.fit(model, LRS2Dataloader,ckpt_path="/data2/alumni/gryang/Leveraging-Self-Supervised-Learning-for-AVSR-main/train-step_0123-wer_0.062.ckpt")
