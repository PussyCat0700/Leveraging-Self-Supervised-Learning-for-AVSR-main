...
def preprocess_sample_list(filesList, face_detector, landmark_detector, queue):
    ...
    cnt = 0
    for file in tqdm(filesList, leave=True, desc="Preprocess", ncols=75): #tdqm 就是个进度条
...
def find_mean_face(filesList, processes, landmark_detector):
    ...
    # multi processing
    queue = Queue()
    queue.put({'sumed_landmarks': np.zeros((68, 2)), 'cnt': 0, 'missed': 0})   #返回68个特征点的位置

    def splitlist(inlist, chunksize):
        return [inlist[x:x + chunksize] for x in range(0, len(inlist), chunksize)]

    filesListSplitted = splitlist(filesList, int((len(filesList) / processes)))   # [[]]这种list 33个list 每个是一个1432的list
